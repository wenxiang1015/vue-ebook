        ...
        methods: {
            ...
            downloadBook(book){
                ...
                return new Promise((resolve,reject) => {
                    download(book,(book)=>{
                        // 由于 create-api 的机制问题,暂不支持 更新 props.
                        toast.remove();//create-api 提供的方法
                        resolve(book);
                    },progressEvent => {
            ...
            setPrivate(){
                ...
                /**
                 * shelfSelected 中的对象都是 shelfList 中的引用
                 * 故修改 shelfSelected 中的对象,shelfList 中的元素也会发生改变
                 */
                saveBookShelf(this.shelfList);
                if(isPrivate){
            ...
            label(tab){
                switch (tab.index) {
                    case 1: return this.isPrivate ? tab.label2 : tab.label;
                    case 2: return this.isDownload ? tab.label2 : tab.label;
